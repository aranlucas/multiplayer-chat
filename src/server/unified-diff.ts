export function applyUnifiedDiff(
  ...
  while (index < lines.length) {
    ...
    if (index >= lines.length || !lines[index].startsWith("+++ "))
      throw new Error("Patch is missing a +++ file header");
    const newPath = patchPath(lines[index].slice(4));
    index += 1;
    if (!newPath || newPath === "/dev/null")
      throw new Error(
        "Deleting files is not supported by the Workers-native patch backend",
      );
    const path = validatePatchPath(newPath);
    const original =
      oldPath === "/dev/null" ? "" : files.get(validatePatchPath(oldPath));
    if (original === undefined)
      throw new Error(`Patch target is not tracked: ${oldPath}`);
    ...
    while (index < lines.length && !lines[index].startsWith("--- ")) {
      ...
      if (hunkStart < sourceIndex || hunkStart > source.length)
        throw new Error(`Patch hunk is out of range for ${path}`);
      ...
      while (
        ...
        if (marker === " " || marker === "-") {
          if (source[sourceIndex] !== value)
            throw new Error(
              `Patch context did not match ${path} at line ${sourceIndex + 1}`,
            );
  ...
  if (!changes.length)
    throw new Error("Patch did not contain a supported unified diff");
  return changes;
}
