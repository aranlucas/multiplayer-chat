...
export class GitHubPullRequestClient {
  ...
  async create(input: PullRequestInput): Promise<PullRequestResult> {
    if (!input.changes.length) throw new Error("There are no shared workspace changes to put in a pull request");
    ...
    const blobs = await Promise.all(input.changes.map(async (change) => ({
      path: change.path,
      sha: (await this.request<{ sha: string }>(`/repos/${writeRepository}/git/blobs`, {
        method: "POST",
    ...
    const commit = await this.request<{ sha: string }>(`/repos/${writeRepository}/git/commits`, {
      method: "POST",
      body: JSON.stringify({ message: input.title, tree: tree.sha, parents: [input.baseCommitSHA] }),
    });
    ...
    const pull = await this.request<{ number: number; html_url: string }>(`/repos/${input.repository}/pulls`, {
      method: "POST",
      body: JSON.stringify({
        title: input.title,
        body: input.body,
        base: input.baseBranch,
        head: writeRepository === baseRepository.full_name ? branch : `${input.login}:${branch}`,
      }),
    });
    return { number: pull.number, url: pull.html_url, branch, repository: input.repository };
  }
  ...
  private async ensureFork(login: string, base: RepositoryResponse): Promise<string> {
    ...
    if (existing) {
      if (existing.full_name !== forkName || existing.parent?.full_name !== base.full_name) {
        throw new Error(`GitHub repository ${forkName} exists but is not a fork of ${base.full_name}`);
      }
      return forkName;
    }
  ...
  private async waitForCommit(repository: string, sha: string): Promise<GitCommitResponse> {
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const commit = await this.request<GitCommitResponse>(`/repos/${repository}/git/commits/${sha}`, {}, true);
  ...
  private async request<T = unknown>(path: string, init: RequestInit, allowNotFound: true): Promise<T | undefined>;
  private async request<T = unknown>(path: string, init: RequestInit = {}, allowNotFound = false): Promise<T | undefined> {
    ...
    if (!response.ok) {
      throw new Error(typeof value.message === "string" ? value.message : `GitHub API request failed (${response.status})`);
    }
    return value as T;
  }
}
...
function branchName(roomID: string): string {
  const room = roomID.toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-").slice(0, 32);
