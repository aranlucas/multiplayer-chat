  ...
  return {
    ...
    async setup(ctx) {
      ...
      await ctx.tool.transform((tools) => {
        tools.add({
          name: "repo_status",
          description: "Return the selected GitHub repository, branch, and pinned commit for this room.",
          input: { type: "object", properties: {}, additionalProperties: false },
          options: { namespace: "relay", codemode: true },
          async execute(_input, tool) {
            await tool.progress({ status: "Preparing the commit-pinned workspace" });
        ...
        tools.add({
          name: "repo_search",
          description: "Search the connected repository for a string or symbol.",
          input: {
            type: "object",
        ...
        tools.add({
          name: "git_diff",
          description: "Read the current uncommitted diff in the shared repository workspace.",
          input: { type: "object", properties: {}, additionalProperties: false },
          options: { namespace: "relay", codemode: true },
          async execute(_input, tool) {
        ...
        tools.add({
          name: "run_tests",
          description: "Run the repository test target and return the terminal transcript.",
          input: {
            type: "object",
            properties: { target: { type: "string", enum: ["auto", "test", "typecheck", "build"] } },
            required: ["target"],
            additionalProperties: false,
          },
          options: { namespace: "relay", codemode: true },
          async execute(input, tool) {
            const target = String((input as { target: string }).target) as "auto" | "test" | "typecheck" | "build";
            await tool.progress({ status: `Running ${target}` });
            return { content: await workspace.runTests(target, (event) => tool.progress({ status: runnerProgress(event) })) };
          },
        });

        tools.add({
          name: "run_command",
          description: "Run an arbitrary shell command in the commit-pinned Microsandbox workspace and stream its terminal output.",
          input: {
            type: "object",
          ...
          async execute(input, tool) {
            ...
            return { content: await workspace.runCommand(command, (event) => tool.progress({ status: runnerProgress(event) })) };
          },
        });
...
function runnerProgress(event: RunnerEvent): string {
  ...
  if (event.type === "result") return `Process exited ${event.exitCode} in ${(event.durationMs / 1_000).toFixed(1)}s`;
  const line = event.data.trim();
  return line ? line.slice(-500) : event.type === "stdout" ? "Command produced output" : "Command produced error output";
}
...
export async function createOpenCode(storage: DurableObjectStorage, env: WorkerEnv, workspace: RepositoryWorkspace) {
  ...
  const liveConfig: OpenCodeWorkerd.Configuration = env.OPENCODE_PROVIDER === "opencode-zen"
    ? ({
      default_agent: "build",
